 ...
 *
 * Uses OpenAI's ChatGPT API with OAuth tokens from ChatGPT Plus/Pro subscription.
...
export class OpenAIAdapter implements IAIProvider {
  ...
  async getRecommendations(watchHistory: WatchHistoryItem[], config: AIConfig): Promise<AIResponse> {
    ...
    if (!extConfig.accessToken) {
      throw new AIProviderError('OpenAI OAuth token required', this.name);
    }
    ...
    if (category) {
      console.log(`[OpenAI] Category: ${category.name} - ${category.description}`);
    }
  ...
  private async getRecommendationsWithOAuth(
    ...
    console.log(`[OpenAI OAuth] Using subscription access with model: ${modelId}`);
  ...
  async validateConfig(config: AIConfig): Promise<boolean> {
    ...
    if (!extConfig.accessToken) {
      throw new AIProviderError('OpenAI OAuth token required for validation', this.name);
    }
    ...
    try {
      ...
      throw new AIProviderError(
        `Invalid OpenAI configuration: ${error instanceof Error ? error.message : 'Unknown error'}`,
        this.name,
        error
      );
    }
